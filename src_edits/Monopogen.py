...
def germline(args):
	...
	with open(args.region) as f_in:
		for line in f_in:
			...
			if args.verbose:
				print(f"> regions to cover: \n{record}")
			if(len(record)==1):
				jobid = record[0]
			if(len(record)==3):
				# this is not IDEAL work on LINUX/macOS because of the colon
				jobid = record[0] + ":" + record[1] + "-" + record[2]
				# but this creates issues as this is not noted in the input files? 
				# seq_id = record[0] + ":" + record[1] + "-" + record[2]
				# jobid = record[0] + "_" + record[1] + "_" + record[2]
